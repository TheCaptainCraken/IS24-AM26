...
/**
 * The LittleModel class is responsible for maintaining a simplified version of the game model in server.
 * It stores information about the game such as points, resources, cards, and other game details.
 * The class provides methods to update and retrieve this information.
 *
 * The LittleModel class uses several HashMaps to store information about points, resources, and cards.
 * It also uses arrays to store information about the player's cards and the common table.
 *
 * The LittleModel class uses a Deck object for each type of card deck in the game.
 * These decks are used to retrieve card information based on card IDs.
 *
 * The LittleModel class provides methods to update the game state, such as updating the player's hand,
 * updating the card on the table, updating the score, updating the resources, and updating the common table.
 * It also provides methods to retrieve information about the game state, such as getting the player's hand,
 * getting the hidden hand of other players, getting the points, getting the resources, and getting the cards on the common table.
 *
 */
public class LittleModel {
    ...
    /**
     * The secretObjectiveCardsToChoose array stores the secret objective cards to choose from.
     */
    ...
    /**
     * The LittleModel constructor initializes the points, resources, myCards, otherPlayersCards, and table HashMaps.
     * It also initializes the startingCardsDeck, objectiveCardsDeck, resourceCardsDeck, and goldCardsDeck.
     */
    public LittleModel() {
        points = new HashMap<>();
     ...
     * The cards are represented by their IDs in the form of an Integer array.
     *
     * @return An array of Integers representing the IDs of the cards in the player's hand.
     */
     ...
     * @return An array of Pairs representing the Kingdom and visibility of the cards in the player's hand.
     */
     ...
     * The points are represented in a HashMap where the key is the player's nickname and the value is the player's points.
     *
     * @return A HashMap representing the nicknames of the players and their corresponding points.
     */
     ...
     * The card is identified by its ID, and its position is represented by a Point object.
     * The side of the card and the turn in which the card was placed are also recorded.
     *
     ...
     * @param side The side of the card (true for gold side, false for resource side).
    ...
    public void updatePlaceCard(String nickname, int id, Point position, boolean side, int turn) {
       ...
       if(position.x == 0 && position.y == 0){
           ...
           //same function as model server, but different purpose. Given a card, we should link it with the other cards.
           for (Corner corner : Corner.values()) {
               ...
               cardOnTable = getAllCards(startingCard, new Point(position.x + xPlaceToCheck, position.y + yPlaceToCheck));
               if(cardOnTable != null) {
     ...
     *
     * This method is used to update the resources of a player with the new set of resources.
     * The resources are represented by a HashMap where the key is the Sign and the value is the quantity of that resource.
     *
     * @param nickname The nickname of the player whose resources are to be updated.
     * @param resources A HashMap representing the resources and their quantities for the player.
     */
     ...
     * The resources are represented in a HashMap where the key is the player's nickname
     * and the value is another HashMap representing the resources and their quantities.
     *
     * @return A HashMap representing the nicknames of the players and their corresponding resources.
     */
    ...
    public ObjectiveCard getObjectiveCard(int id){
        //we got  objective cards array, so I need to correct the index. ObjectiveCards index are form 1 to 16.
        id = id - 1;
     ...
     * The ID is adjusted by subtracting 97 to match the index in the startingCardsDeck.
     *
    ...
    public PlayedCard getStartingCard(int id, boolean side){
        //we got starting cards array, so I need to correct the index. StartingCards index are form 97 to 102.
        id = id - 97;
     ...
     * If the ID is greater than or equal to 97, it is considered a starting card and the getStartingCard method is called.
     * If the ID is less than 57, it is considered a resource card and is retrieved from the resourceCardsDeck.
     * Otherwise, it is considered a gold card and is retrieved from the goldCardsDeck.
     *
     * @param id The ID of the card.
     * @param side The side of the card (true for gold side, false for resource side).
     ...
     *
     * This method is used to update the common table with the new set of resource cards, gold cards, and the head decks for both types of cards.
     * The resource cards and gold cards are represented by Integer arrays where each Integer is the ID of a card.
     ...
     * @param goldCards An array of Integers representing the IDs of the gold cards on the table.
     * @param resourceCardOnDeck A Kingdom object representing the head deck of resource cards.
     * @param goldCardOnDeck A Kingdom object representing the head deck of gold cards.
     */
    public void updateCommonTable(Integer[] resourceCards, Integer[] goldCards, Kingdom resourceCardOnDeck, Kingdom goldCardOnDeck) {
     ...
     * @return A CardClient object representing the card found, or null if no card is found at the given position.
     */
     ...
     *
     * This method is used to recursively find a card in the game based on its position.
     * The position is represented by a Point object.
     * The method uses a Stack to keep track of the cards that have been visited during the search.
     *
     * @param card The current card being checked.
     * @param position The position of the card to find.
     * @param stack A Stack of CardClient objects representing the cards that have been visited during the search.
     * @return A CardClient object representing the card found, or null if no card is found at the given position.
     */
     ...
     * The card is identified by its index in the secretObjectiveCardsToChoose array.
     *
     * @param indexCard The index of the secret objective card in the secretObjectiveCardsToChoose array.
     */
     ...
     *
     * This method is used to update the secret objective cards that the player can choose from.
     * The cards are represented by an Integer array where each Integer is the ID of a card.
     *
     * @param secretObjectiveCardsToChoose An array of Integers representing the IDs of the secret objective cards to choose from.
     */
     ...
     *
     * This method is used to retrieve the secret objective cards that the player can choose from.
     * The cards are represented by an Integer array where each Integer is the ID of a card.
     *
     * @return An array of Integers representing the IDs of the secret objective cards to choose from.
     */
     ...
     * The cards are represented by an Integer array where each Integer is the ID of a card.
     *
     * @return An array of Integers representing the IDs of the common objective cards.
     */
     ...
     * The card is represented by an Integer where the Integer is the ID of the card.
     *
     ...
     * The players are represented by a HashMap where the key is the player's nickname and the value is the player's color.
     *
     ...
     * The cards are represented by an Integer array where each Integer is the ID of a card.
     *
     * @return An array of Integers representing the IDs of the gold cards on the table.
     */
     ...
     * The cards are represented by an Integer array where each Integer is the ID of a card.
     *
     * @return An array of Integers representing the IDs of the resource cards on the table.
     */
     ...
     * The list is generated by finding all the cards linked to the card with the given nickname.
     *
     ...
     *
     * @param startingCard The starting card from which to start the search.
     * @return An ArrayList of CardClient objects representing the cards linked to the starting card.
     */
     ...
     *
     * This method is used to recursively find all cards in the game linked to the starting card.
     * The method uses a Stack to keep track of the cards that have been visited during the search.
     *
     * @param card The current card being checked.
     * @param stack A Stack of CardClient objects representing the cards that have been visited during the search.
